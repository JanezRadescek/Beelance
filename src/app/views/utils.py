...
def get_nav_bar(session):
    ...
    result += '    <li><a href="reset">Reset Password</a></li>'
    result += ' </ul>'
    result += '</nav>'
    return result
...
def hash_string(input_string, salt):
    binary_hash = hashlib.pbkdf2_hmac('sha256', input_string.encode('utf-8'), salt.encode(), 100000)
    string_hash = binascii.hexlify(binary_hash).decode()
    return string_hash
